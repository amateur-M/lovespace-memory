...
3. **`lovespace-ai` 模块**：承载 LLM 抽象（`LLMProvider`）、通义千问（`QwenProvider`，阿里云 DashScope Java SDK）、OpenAI（Spring AI `OpenAiChatModel` + `ChatClient`）、`LlmRouter`、`AiChatService` 与 `POST /api/v1/ai/chat`；**`DashScopeEmbeddingModel`** 实现 Spring AI **`EmbeddingModel`**（DashScope `TextEmbedding`，默认 `text-embedding-v2`，与聊天共用 `spring.ai.dashscope.api-key`），在 **`lovespace.ai.embedding.provider=dashscope`**（默认）且无其它 `EmbeddingModel` Bean 时由 **`DashScopeEmbeddingConfiguration`** 注册；配置 `lovespace.ai.embedding.*`；`application.yml` **排除** `OpenAiEmbeddingAutoConfiguration`，不依赖 OpenAI 官方嵌入自动配置。
   ...
   **RAG/Milvus**（`spring-ai-starter-vector-store-milvus`、Redis 多轮等）与 LLM **同在 `lovespace-ai`**，由 **`lovespace-user`** 依赖 **`lovespace-ai`** 打入 fat jar；**Milvus**（`spring-ai-starter-vector-store-milvus`）集合名由 **`spring.ai.vectorstore.milvus.collection-name`** 配置（默认与常量 **`love_knowledge_base`** 对齐）；业务侧原生客户端封装 Bean 名为 **`lovespaceMilvusClient`**（`com.meng.lovespace.ai.milvus.MilvusClient`），**避免**与 Spring AI 自动配置中 **`milvusClient`**（`MilvusServiceClient`）重名；**`MilvusSchemaService`** 在 **`lovespace.milvus.ensure-love-knowledge-schema`**（默认 true）时于启动按 Spring AI `MilvusVectorStore` 同款 schema 建集合并建索引、`loadCollection`；**不再**使用 `lovespace.ai.rag.enabled` 与 **`RagMilvusAutoConfigurationExclusionEnvironmentPostProcessor`**。

   **RAG 优化策略**（2026-05 实施）：
   - **Embedding 缓存**：`CachedEmbeddingModel` 装饰器模式包装底层 `DashScopeEmbeddingModel`，Redis 缓存 query → embedding 结果；标准化 query（trim/lowercase/截断）后 SHA-256 作为 key；TTL 24h；预期降低 API 调用成本 60%。
   - **Prompt 压缩**：`PromptCompressor` 组件在 `LoveQAService.prepareChat()` 中执行——检索片段去重（Jaccard 相似度 0.85 阈值）、上下文长度限制（8000 字符）、历史对话精简（保留最近 10 轮）；预期降低 Token 消耗 20-40%。
   - **检索可视化**：`LoveQaStreamCallback` 新增 `onRetrieved(List<RetrievedChunk>)` 回调，SSE 新增 `retrieved` 事件；前端 `AILoveQA.tsx` 展示引用来源卡片，提升用户信任度。
   - **Latency 埋点**：`RagMetricsCollector` + `RagTimer` 记录全流程各阶段耗时（Embedding/检索/Prompt构建/LLM首字节/LLM总耗时/持久化）；DEBUG 日志输出，便于性能分析与瓶颈定位。

   可选第二集合 **`travel_poi_embeddings`**（`lovespace.milvus.ensure-travel-poi-schema` 骨架）。**恋爱 RAG**：**`LoveQAController`** 为 **`@RestController`** 组件扫描注册；**`LoveQAService`** 实现 **`LoveQaChatFacade`**，在 **`LoveQaRagBeansConfiguration`** 中声明 **`@Bean`**，且 **`@DependsOn("milvusSchemaService")`** 以保证集合就绪顺序；**`POST /api/v1/ai/love-qa/ingest|chat`** 依赖 **`VectorStore` + `EmbeddingModel` + Redis**。**情侣旅游规划**：`TravelPlannerService` + `POST /api/v1/ai/travel/plan`，输出 JSON（`TravelPlanJsonSchema`）。父 POM 引入 `spring-ai-bom`（与 Spring Boot 3.4.x 对齐）；官方 `spring-ai-bom` 不含 DashScope，通义侧不引入 `spring-ai-alibaba` 以免与 Boot 版本冲突
...
4. **恋爱知识库 RAG**：`POST /api/v1/ai/love-qa/ingest|ingest/file|ingest/url`（**MySQL 台账** `love_qa_documents` + **Milvus 双写**，脚本 `love_qa_documents.sql`；入库返回 `documentId`/`chunkCount`；Milvus chunk metadata 含 `documentId`、`chunkIndex`、`totalChunks`、`scope`、`ingestedAt`）、`POST /api/v1/ai/love-qa/chat`（**非流式**：检索 + LLM + **Redis 多轮记忆** + **MySQL 每轮持久化** `love_qa_conversations` / `love_qa_messages`，脚本 `love_qa.sql`）、**`POST /api/v1/ai/love-qa/chat/stream`（SSE 流式，语义与 `/chat` 一致，流结束后同样 `appendChatRound`）**；**文档台账** `GET/DELETE/POST .../love-qa/documents`（`LoveQaDocumentController` + `LoveQaDocumentService`）；`GET .../love-qa/conversations`、`GET .../love-qa/conversations/{id}/messages` 供历史查看；`LoveQAController` 在 `lovespace-user`（需 JWT），仅在存在 `LoveQaChatFacade` Bean（RAG 启用且 `VectorStore` 等就绪）时注册；`LoveQAHistoryController` / `LoveQaDocumentController` 不直接依赖 Milvus 删除逻辑（经 Facade）；会话键 `lovespace:love-qa:conv:{id}`，TTL 与最大轮数见 `lovespace.ai.rag.*`；**入库 coupleId 校验**（`LoveQaIngestValidator`）：默认 `allow-global-ingest: false`，未绑定情侣拒绝（40093）；`coupleId` 须与 `getCoupleInfo.bindingId` 一致且 `findActiveOrFrozenMembership` 通过（40393）；未传 `coupleId` 时自动解析为当前 bindingId；**多轮上下文**由 **`LLMProvider.chatWithSystemAndHistory(Streaming)`** 以 **system（RAG 指令+检索片段）+ 多条 user/assistant + 本轮 user** 调用模型（避免把历史仅塞进超长 system）；**Redis 无会话但 MySQL 有历史**时 **`restoreRedisSessionIfMissing`** 在 `/chat` 与 `/chat/stream` 调用 Facade 前回填；向量维度与嵌入模型见 **`lovespace.ai.embedding.*`** 与 **`spring.ai.vectorstore.milvus.embedding-dimension`**（须一致）；部署需 **`SPRING_AI_DASHSCOPE_API_KEY`**（嵌入与通义聊天共用，RAG 场景下）；经 **nginx** 等代理 SSE 时注意 **`proxy_buffering off`** 等以免增量被缓冲

   **RAG 优化增强**（2026-05）：
   - **Embedding 缓存**：`CachedEmbeddingModel` 实现 Redis 缓存，query 标准化后 SHA-256 为 key；预期 Embedding API 调用成本降低 60%，延迟从 150ms 降至 5ms（命中时）。
